...
class ApiCommands():
	...
	async def get_character_level(self, api_key, character_name):
		...
		try:
			...
			await error_logging().log_error(traceback.format_exc(), 'api_commands: get_character_level', str(member), self.bot)
	...
	async def get_skill_ids(self, char_name, api_key, game_type):
		...
		#print(data)
			...
			await error_logging().log_error(traceback.format_exc(), 'api_commands: get_skill_ids', str(member), self.bot)
	...
	async def get_skill_data(self, skill_dict):
		...
		# 		utilities = utilities + ', ' + str(data_skills[x]['name'])
			...
			await error_logging().log_error(traceback.format_exc(), 'api_commands: get_skill_data', str(member), self.bot)
